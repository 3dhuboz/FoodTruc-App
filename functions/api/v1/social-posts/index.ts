...
export const onRequest = async (context: any) => {
  ...
  try {
    ...
    if (request.method === 'GET') {
      const { results } = await db.prepare('SELECT * FROM social_posts ORDER BY scheduled_for DESC').all();
    ...
    if (request.method === 'POST') {
      ...
      await db.prepare(
        `INSERT OR REPLACE INTO social_posts (id, platform, content, image, scheduled_for, status, hashtags)
         VALUES (?, ?, ?, ?, ?, ?, ?)`
      ).bind(id, post.platform, post.content, post.image || null,
